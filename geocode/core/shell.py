...
from geocode.helpers.index import token_frequency
from geocode.helpers.search import preprocess_query
from geocode.helpers.text import compare_str
...
class Cmd(cmd.Cmd):
    ...
    intro = (
        white("\nWelcome to the Geocode shell o/\n")
    ...
    def do_CONFIG(self, name):
        """Inspect loaded Geocode config. Output all config without argument.
        CONFIG [CONFIG_KEY]"""
        if not name:
...
def register_command(subparsers):
    parser = subparsers.add_parser("shell", help="Run a shell to inspect Geocode")
    parser.set_defaults(func=invoke)
    parser = subparsers.add_parser(
        "pyshell", help="Run a python shell with Geocode config"
    )
